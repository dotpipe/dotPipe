...
const dotPipe = {
    ...
    // --- runInline updated with shell + macro support ---
    runInline: async function (key) {
        ...
        for (let i = 0; i < segments.length; i++) {
            ...
            // --- Macro operations: .class[n], .class[], .class[x,y], .class[start:end:step]
            if (m = /^([#.][\w-]+)(?:\[(.*?)\])?\.(.+?):(.+)$/.exec(seg)) {
                const [, selector, indexExpr, property, rawVal] = m;
                const value = rawVal.startsWith('!') ? (currentShell || entry).dpVars[rawVal.slice(1)] : rawVal;
                ...
                if (indexExpr === "") {
                ...
                } else if (indexExpr && indexExpr.includes(",")) {
                    // [x,y,z] → explicit indices
                    const indices = indexExpr.split(",").map(n => parseInt(n.trim(), 10)).filter(n => !isNaN(n));
                    indices.forEach(idx => {
                        if (idx < 0) idx = len + idx; // negative index
                        const targetEl = elems[idx];
                        if (targetEl) this.setProperty(targetEl, property, value);
                    });
                } else if (indexExpr && indexExpr.includes(":")) {
                    ...
                    if (end !== null && end < 0) end = len + end;

                    if (end === null) {
                        // [start:count] form → yield count elements
    ...
    // Helper for property/class/style assignments
    setProperty: function (el, property, value) {
        if (property === "classList.add") {
