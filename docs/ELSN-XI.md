...
If `allowBrowserWrites="true"` is explicitly enabled, the paired HttpOnly session may also `PUT action=write`, `PUT action=patch`, `PUT action=pause`, or `PUT action=resume`. Pause/resume requires the endpoint name in `path` and is still constrained by the server's allow rules. A successful browser write or lifecycle change rotates the session cookie and produces a new site digest. The installer leaves this disabled by default. The Webapp Operations dashboard uses this opt-in channel for real status and endpoint lifecycle controls; it does not claim to broadcast notices or refresh client sessions unless a future server capability is added.
...
`xi signal` sends the client snapshot digest over the signed channel. A mismatch returns `rebaseRequired` without changing either side. `xi signal --rebase` is an explicit opt-in: it uploads the client's files through the normal signed `write` path and verifies the digest again. The server must have both `allowWrites="true"` and `allowClientRebase="true"`; both are false by default. This never deletes server-only files, and each uploaded file remains subject to the XML allow/deny rules and audit log.
