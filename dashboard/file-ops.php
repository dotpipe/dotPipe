...
function file_ops_path(string $value, string $root, array $extensions): string {
    ...
    if (!in_array(strtolower(pathinfo($value, PATHINFO_EXTENSION)), $extensions, true)) file_ops_fail(403, 'file type is not editable');
...
if (!is_array($input) || !in_array($action, ['create', 'copy', 'move', 'delete', 'write', 'undo', 'list-history', 'list-trash'], true)) file_ops_fail(400, 'unsupported file operation');
...
if ($action === 'undo') {
    ...
    if ($kind === 'delete') { $source = file_ops_path($item['path'], $root, $extensions); $trash = file_ops_trash_path((string) ($item['trash'] ?? ''), $root); if (file_exists($source) || is_link($source)) file_ops_fail(409, 'original path is already occupied'); if (!is_dir(dirname($source)) && !mkdir(dirname($source), 0750, true)) file_ops_fail(500, 'could not restore parent directory'); if (!is_file($trash) || !rename($trash, $source)) file_ops_fail(500, 'could not restore file'); }
    elseif ($kind === 'create' || $kind === 'copy') { $target = file_ops_path($kind === 'create' ? $item['path'] : $item['destination'], $root, $extensions); if (is_file($target) && !unlink($target)) file_ops_fail(500, 'could not undo file'); }
    ...
    elseif ($kind === 'move') { $source = file_ops_path($item['source'], $root, $extensions); $destination = file_ops_path($item['destination'], $root, $extensions); if (file_exists($source)) file_ops_fail(409, 'original move path is already occupied'); if (!rename($destination, $source)) file_ops_fail(500, 'could not undo move'); }
...
$source = file_ops_path((string) ($input['source'] ?? $input['path'] ?? ''), $root, $extensions); if (!is_file($source)) file_ops_fail(404, 'source file not found');
if ($action === 'delete') { if (is_link($trashRoot) || (is_dir($trashRoot) && (($resolvedTrash = realpath($trashRoot)) === false || ($resolvedTrash !== $root && !str_starts_with($resolvedTrash, $root . DIRECTORY_SEPARATOR))))) file_ops_fail(403, 'trash directory escapes the workspace'); $id = bin2hex(random_bytes(6)); $trashDir = $trashRoot . DIRECTORY_SEPARATOR . $id; if (!is_dir($trashDir) && !mkdir($trashDir, 0750, true)) file_ops_fail(500, 'could not create trash stage'); $trash = $trashDir . DIRECTORY_SEPARATOR . 'file'; if (!rename($source, $trash)) file_ops_fail(500, 'could not move file to trash'); $items = file_ops_history($root); $items[] = ['id' => $id, 'time' => gmdate('c'), 'action' => 'delete', 'path' => file_ops_rel($source, $root), 'trash' => file_ops_rel($trash, $root)]; file_ops_save_history($root, $items); echo json_encode(['ok' => true, 'id' => $id]); exit; }
$destination = file_ops_path((string) ($input['destination'] ?? ''), $root, $extensions); if (file_exists($destination)) file_ops_fail(409, 'destination already exists'); if (!is_dir(dirname($destination)) && !mkdir(dirname($destination), 0750, true)) file_ops_fail(500, 'could not create destination directory'); $ok = $action === 'copy' ? copy($source, $destination) : rename($source, $destination); if (!$ok) file_ops_fail(500, "could not {$action} file"); $id = file_ops_record($root, ['action' => $action, 'source' => file_ops_rel($source, $root), 'destination' => file_ops_rel($destination, $root)]); echo json_encode(['ok' => true, 'id' => $id, 'path' => file_ops_rel($destination, $root)]);
