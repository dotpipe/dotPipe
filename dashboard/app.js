(() => {
  ...
  async function loadSshProfiles() { const response = await fetch(`${sshBrokerUrl()}/profiles`, { headers: { 'X-XI-XI-Local-Token': $('sshBrokerToken').value.trim() }, cache: 'no-store' }); const result = await response.json().catch(() => ({})); if (!response.ok) throw new Error(result.error || `HTTP ${response.status}`); const select = $('sshSite'); select.replaceChildren(...(Array.isArray(result.sites) ? result.sites : []).map(site => { const option = document.createElement('option'); option.value = site.id; option.textContent = `${site.name} · allow: ${(site.allow || []).join(', ')}`; return option; })); if (!select.options.length) { const option = document.createElement('option'); option.value = 'default'; option.textContent = 'Default site'; select.appendChild(option); } }
  async function testSshBroker() { try { const url = `${sshBrokerUrl()}/health`; const response = await fetch(url, { headers: { 'X-XI-XI-Local-Token': $('sshBrokerToken').value.trim() }, cache: 'no-store' }); const result = await response.json().catch(() => ({})); if (!response.ok) throw new Error(result.error || `HTTP ${response.status}`); await loadSshProfiles(); $('sshConsoleState').textContent = 'Broker connected'; $('sshConsoleOutput').textContent = JSON.stringify(result, null, 2); } catch (error) { $('sshConsoleState').textContent = error.message; $('sshConsoleOutput').textContent = `SSH broker error: ${error.message}`; } }
  async function runSshOperation() { try { const response = await fetch(`${sshBrokerUrl()}/run`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-XI-XI-Local-Token': $('sshBrokerToken').value.trim() }, body: JSON.stringify({ site: $('sshSite').value, command: sshCommand() }), cache: 'no-store' }); const result = await response.json().catch(() => ({})); if (!response.ok) throw new Error(result.error || result.stderr || `HTTP ${response.status}`); $('sshConsoleState').textContent = `Completed ${$('sshOperation').value} on ${result.site || $('sshSite').value}`; $('sshConsoleOutput').textContent = result.stdout || '(no output)'; addAudit('SSH operation completed', `${result.site || $('sshSite').value} · ${$('sshOperation').value}`); } catch (error) { $('sshConsoleState').textContent = error.message; $('sshConsoleOutput').textContent = `SSH operation failed: ${error.message}`; } }
