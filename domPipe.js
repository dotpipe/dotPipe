/**
 * domPipe.js – Dynamic Web Components & Attribute Framework
 * 
 * USAGE INSTRUCTIONS:
 * 
 ...
 * <timer>              Auto-refresh content at intervals.
 * <timed>              Backward-compatible alias for <timer>.
 ...
 * delay                For <timer>/<timed>, <carousel>; refresh/slide interval (ms).
 ...
 *   - This allows extending domPipe with ANY custom JS logic.
 * ============================================================
 * EXAMPLES
 * 
 ...
 * SYSTEM FLOW:
 * 
 * 1. domPipe.register(selector) scans for elements with 'inline' attributes.
 * 2. Each element gets a dpVars object for variable storage.
 * 3. Inline macros are executed manually with domPipe.runInline(id).
 ...
 * ──────────────────────────────────────────────────────────────
 * SYSTEM FLOW:
 * - On DOMContentLoaded, domPipe processes all supported custom tags.
 ...
 * 
 * (c) domPipe.js – https://github.com/dompipe/domPipe
 */

// Public domPipe attributes keep their original spelling for backwards
// compatibility.  The matching data-* form is maintained automatically so
// markup can be migrated incrementally without creating two APIs.
const DOMPIPE_ATTRIBUTE_NAMES = Object.freeze([
    'inline', 'ajax', 'insert', 'query', 'callback', 'callback-class', 'modal',
    'file', 'directory', 'set', 'get', 'delete', 'x-toggle', 'tool-tip',
    'modal-tip', 'copy', 'remove', 'display', 'headers', 'form-class',
    'action-class', 'event', 'options', 'sources', 'style', 'tab', 'tabs',
    'login-page', 'registration-page', 'css-page', 'validate', 'pages',
    'count', 'percents', 'height', 'width', 'delay', 'interval', 'file-order',
    'file-index', 'mode', 'turn', 'turn-index', 'boxes', 'sort', 'page-size',
    'lazy-load', 'target', 'bind', 'use-id', 'input-width', 'input-height',
    'search-delay', 'csv-as', 'csv-class', 'order-id', 'node', 'append',
    'vertical', 'iter', 'index', 'source', 'type', 'server', 'token',
    'idempotency-key'
]);

const DOMPIPE_CUSTOM_ELEMENTS = new Set([
    'pipe', 'cart', 'item', 'dyn', 'search', 'csv', 'tabs', 'login',
    'checkout', 'carousel', 'columns', 'timer', 'timed', 'refresh',
    'order-confirmation', 'lnk', 'csv-foreach'
]);

function domPipeDataName(name) {
    return name.startsWith('data-') ? name : `data-${name}`;
}

// Remote definitions can supply URLs and attribute names. Keep the existing
// relative/HTTP(S) behavior, but prevent scriptable URL schemes and inline
// event-handler attributes from turning JSON/API content into code execution.
function isDomPipeSafeUrl(value, { allowMailto = false, allowData = false } = {}) {
    const raw = String(value ?? '').trim();
    if (!raw || raw.startsWith('#')) return false;
    if (/^javascript\s*:/i.test(raw) || /^vbscript\s*:/i.test(raw)) return false;
    if (allowData && /^data:image\/(?:png|gif|jpe?g|webp|svg\+xml);base64,/i.test(raw)) return true;
...
function setDomPipeSafeAttribute(element, name, value) {
    const attribute = String(name ?? '').trim();
    if (!attribute || /^on/i.test(attribute) || attribute.toLowerCase() === 'srcdoc') return false;
    const stringValue = String(value ?? '');
    if (['href', 'src', 'action', 'formaction', 'poster'].includes(attribute.toLowerCase()) && !isDomPipeSafeUrl(stringValue, { allowMailto: attribute.toLowerCase() === 'href', allowData: attribute.toLowerCase() === 'src' })) return false;
    try { element.setAttribute(attribute, stringValue); return true; } catch (error) { return false; }
}

function domPipeCssValues(value) {
    const values = Array.isArray(value) ? value : String(value ?? '').split(/[;\n]/);
    return values.flatMap(item => Array.isArray(item) ? domPipeCssValues(item) : [String(item ?? '').trim()]).filter(Boolean);
}

function appendDomPipeStylesheets(value, target) {
    if (!target || typeof target.appendChild !== 'function') return 0;
    let added = 0;
    domPipeCssValues(value).forEach(href => {
        if (!isDomPipeSafeUrl(href)) return;
        const existing = Array.from(target.querySelectorAll?.('link[data-dompipe-css]') || []).find(link => link.dataset.dompipeCss === href);
        if (existing) return;
        const link = document.createElement('link');
        link.rel = 'stylesheet';
        link.href = href;
        link.dataset.dompipeCss = href;
...
function isDomPipeElement(element) {
    return element && element.nodeType === Node.ELEMENT_NODE && (
        DOMPIPE_CUSTOM_ELEMENTS.has(element.tagName.toLowerCase()) ||
        element.hasAttribute('inline') || element.hasAttribute('data-inline') ||
        element.classList.contains('mouse')
    );
}

function ensureActiveDomPipeIds(root = document) {
    if (!root || typeof root.querySelectorAll !== 'function') return;
    const candidates = root.nodeType === Node.ELEMENT_NODE
        ? [root, ...root.querySelectorAll('*')]
        : Array.from(root.querySelectorAll('*'));
    const activeTags = candidates.filter(element =>
        DOMPIPE_CUSTOM_ELEMENTS.has(element.tagName.toLowerCase()) ||
        element.hasAttribute('inline') || element.hasAttribute('data-inline') ||
        element.classList.contains('mouse')
    );
    ...
    activeTags.forEach(element => {
        ...
        if (originalId && idCounts.get(originalId) === 1) return;

        let nextId;
        do {
            nextId = `domPipe-${tagName}-${generatedIndex++}`;
        } while (usedIds.has(nextId));
        element.id = nextId;
        usedIds.add(nextId);
        idCounts.set(nextId, 1);
        if (originalId) {
            console.warn(`[domPipe] Duplicate active tag id "${originalId}" repaired as "${nextId}".`, element);
        } else {
            console.warn(`[domPipe] Active <${tagName}> tag was missing an id; assigned "${nextId}".`, element);
        }
    });
}

function syncDomPipeAttributes(root = document, preferredAttribute = null) {
    if (!root || typeof root.querySelectorAll !== 'function') return;

    ensureActiveDomPipeIds(root);
    ...
    elements.forEach(element => {
        DOMPIPE_ATTRIBUTE_NAMES.forEach(name => {
            const dataName = domPipeDataName(name);
            const legacyValue = element.getAttribute(name);
            const dataValue = element.getAttribute(dataName);

            if (legacyValue === null && dataValue === null) return;

            // The data-* spelling is the runtime's canonical form. When an
            // author edits the original spelling, the mutation observer passes
            // that spelling here so the two forms remain equivalent. A data-*
            // edit wins when it is the changed spelling; initial markup keeps
            // the original spelling's established value for compatibility.
            const preferredValue = preferredAttribute === dataName
                ? dataValue
                : preferredAttribute === name
                    ? legacyValue
                    : legacyValue ?? dataValue;
            if (preferredValue === null) return;
            if (dataValue !== preferredValue) element.setAttribute(dataName, preferredValue);
            if (isDomPipeElement(element) && legacyValue !== preferredValue) element.setAttribute(name, preferredValue);
        });
    });
}

function getDomPipeAttribute(element, name) {
    if (!element || typeof element.getAttribute !== 'function') return null;
    const dataName = domPipeDataName(String(name ?? '').trim());
    return element.getAttribute(dataName) ?? element.getAttribute(String(name ?? '').trim());
}

function setDomPipeAttribute(element, name, value) {
    if (!element || typeof element.setAttribute !== 'function') return false;
    const normalizedName = String(name ?? '').trim().replace(/^data-/, '');
    if (!DOMPIPE_ATTRIBUTE_NAMES.includes(normalizedName)) return false;
    const stringValue = String(value ?? '');
    element.setAttribute(domPipeDataName(normalizedName), stringValue);
    if (isDomPipeElement(element)) element.setAttribute(normalizedName, stringValue);
    return true;
}

function removeDomPipeAttribute(element, name) {
    if (!element || typeof element.removeAttribute !== 'function') return false;
    const normalizedName = String(name ?? '').trim().replace(/^data-/, '');
    if (!DOMPIPE_ATTRIBUTE_NAMES.includes(normalizedName)) return false;
    element.removeAttribute(domPipeDataName(normalizedName));
    element.removeAttribute(normalizedName);
    return true;
}

function observeDomPipeAttributes() {
    if (typeof MutationObserver === 'undefined' || !document.documentElement) return;
    if (document.documentElement.dataset.domPipeAttributeObserver === 'true') return;

    const observedAttributes = DOMPIPE_ATTRIBUTE_NAMES.flatMap(name => [name, domPipeDataName(name)]);
    const observer = new MutationObserver(mutations => {
        mutations.forEach(({ target, attributeName }) => syncDomPipeAttributes(target, attributeName));
    });
    ...
    document.documentElement.dataset.domPipeAttributeObserver = 'true';
}
        ...
        const observer = new MutationObserver(mutations => {
            mutations.forEach(mutation => {
                mutation.addedNodes.forEach(node => {
                    if (node.nodeType === 1) {
                        syncDomPipeAttributes(node);
                        processMarkdownContainers();
...
document.addEventListener("DOMContentLoaded", function () {
    try {
        syncDomPipeAttributes(document);
        observeDomPipeAttributes();
        const bodyText = document.body?.textContent?.trim() || '';
        const looksLikeJson = bodyText.startsWith('{') || bodyText.startsWith('[');
        if (document.body != null && looksLikeJson) {
            const irc = JSON.parse(bodyText);
    ...
    generateNonce().then(nonce => {
        ...
        meta.content = `default-src 'self'; script-src 'self' 'nonce-${pageNonce}'; style-src 'self' 'unsafe-inline' 'unsafe-hashes'; img-src 'self' data: https: http:; connect-src https: http: 'self'; child-src 'none'; object-src 'none'`;
    ...
    for (const key in domPipe.matrix) {
        ...
        if (events) {
            events.split(';').forEach(ev => {
                el.addEventListener(ev.trim(), () => domPipe.runInline(key));
            });
        }
    }
...
const domPipe = {
    ...
    runShellOpen: function (targetId, shellName, entry) {
        ...
        console.log(`[domPipe] Opened shell "${shellName}" on target "${targetId}"`);
        return entry.shells[shellName];
    },

    runShellClose: function (shellName, entry) {
        if (entry.shells && entry.shells[shellName]) {
            console.log(`[domPipe] Closed shell "${shellName}"`);
            delete entry.shells[shellName];
            return true;
        } else {
            console.warn(`[domPipe] Tried to close unknown shell: "${shellName}"`);
            return false;
        }
    },
    ...
    runInline: async function (key) {
        ...
        for (let i = 0; i < segments.length; i++) {
            ...
            const EXCL_PLACEHOLDER = '__DOMPIPE_EXCL__';
            ...
            if (m = /^([#\.]?[a-zA-Z0-9_\-]+)(\[\s*([a-zA-Z0-9_\-]+)\s*\])\s*:(.+)$/.exec(seg)) {
                ...
                rawValue = rawValue.replace(/__DOMPIPE_EXCL__/g, '!');
                ...
                if (typeof rawValue === 'string') {
                    ...
                    rawValue = domPipe.parseValue(rawValue);
                }
            ...
            if (m = /^([#\.\$]?[\w\-]+)(?:\[(.*?)\])?\.(text|style|[a-zA-Z\-]+)\:(.+)$/.exec(seg)) {
                ...
                const elems = domPipe.getElementsForSelector(selector, indexExpr);
                if (!elems || elems.length === 0) {
                    // nothing to apply to; warn optionally
                    // console.warn("[domPipe] operator: selector matched no elements:", selector, indexExpr);
            ...
            if (m = /^\&([a-zA-Z0-9_]+):(.+)$/.exec(seg)) {
                ...
                if (initMatch) {
                    ...
                    if (s.dpVars[existingVar] === undefined) {
                        s.dpVars[existingVar] = domPipe.parseValue(defaultVal);
                    }
                    currentValue = s.dpVars[existingVar];
                } else {
                    s.dpVars[varName] = domPipe.parseValue(value);
            ...
            if (m = /^([#\.\$]?[\w\-]+)\.([a-zA-Z\-]+):(.+)$/.exec(seg)) {
                ...
                // Restore literal exclamation marks
                value = value.replace(/__DOMPIPE_EXCL__/g, '!');
                ...
                if (typeof value === 'string') {
                    ...
                    value = domPipe.parseValue(value);
                }

                const el = domPipe.resolveElement(selector);
                if (!el) {
                    console.warn("[domPipe] selector not found:", selector);
                    continue;
                }
            ...
            if (m = /^\$([a-zA-Z0-9_-]+)(?::(.+))?$/.exec(seg)) {
                ...
                if (targetEl) {
                    ...
                    // Restore literal exclamation marks
                    if (typeof value === 'string') value = value.replace(/__DOMPIPE_EXCL__/g, '!');
    ...
    runShell: async function (shell) {
        ...
        for (let seg of shell.segments) {
            ...
            if (m = /^\&([a-zA-Z0-9_]+):(.+)$/.exec(seg)) {
                ...
                if (initMatch) {
                    ...
                    if (shell.dpVars[existingVar] === undefined) {
                        shell.dpVars[existingVar] = domPipe.parseValue(defaultVal);
                    }
                    currentValue = shell.dpVars[existingVar];
                } else {
                    shell.dpVars[varName] = domPipe.parseValue(value);
            ...
            if (m = /^([a-zA-Z0-9_+\-]+):?(.*)$/.exec(seg)) {
                ...
                if (typeof domPipe.verbs[verb] === 'function') {
                    currentValue = await domPipe.verbs[verb](...resolvedParams, shell);
                }
                continue;
            }
        }
    ...
    loadVariablesFromFile: async function (url, key) {
        try {
            ...
            console.log('[domPipe] Loaded variables from', url, data);
            return data;
        } catch (err) {
            console.error('[domPipe] Failed to load JSON:', err);
            return null;
        }
    },
    ...
    verbs: {
        ...
        call: async function (fnName, ...args) {
            return await domPipe.runCall(fnName, ...args);
        }
    },
    ...
    runCall: async function (fnName, ...args) {
        ...
        if (!fn) {
            console.error(`[domPipe] runCall: function "${fnName}" not found`);
            return;
        }
        ...
        try {
            ...
            console.log(`[domPipe] runCall executed "${fnName}" with`, args, "→", result);
            return result;
        } catch (err) {
            console.error(`[domPipe] runCall error in "${fnName}"`, err);
            return;
        }
    }
};

const DOMPIPE_API_REGISTRY_STORAGE_KEY = 'domPipe.api-workspace.v1';

function normalizeDomPipeApi(api) {
    if (!api || !api.id || !api.url || !api.method) return null;
    return {
        id: String(api.id),
        ...
        headers: api.headers || '{}',
        body: api.body || '',
        serverUrl: String(api.serverUrl || ''),
        idempotencyKey: String(api.idempotencyKey || ''),
        source: api.source || 'runtime'
    };
}

function readDomPipeApiRegistry() {
    try {
        const raw = localStorage.getItem(DOMPIPE_API_REGISTRY_STORAGE_KEY) || '[]';
        if (raw.length > 512 * 1024) return [];
        const saved = JSON.parse(raw);
        return Array.isArray(saved) ? saved.map(normalizeDomPipeApi).filter(Boolean) : [];
    } catch (error) {
        console.warn('[domPipe] API registry could not be read:', error);
        return [];
    }
}

function emitDomPipeApiEvent(type, detail) {
    window.dispatchEvent(new CustomEvent(type, { detail }));
}

domPipe.apiRegistry = new Map(readDomPipeApiRegistry().map(api => [api.id, api]));
domPipe.registerApis = function (apis, options = {}) {
    const normalized = (Array.isArray(apis) ? apis : [apis]).map(normalizeDomPipeApi).filter(Boolean);
    normalized.forEach(api => this.apiRegistry.set(api.id, api));
    if (options.persist !== false) {
        localStorage.setItem(DOMPIPE_API_REGISTRY_STORAGE_KEY, JSON.stringify(Array.from(this.apiRegistry.values())));
    }
    emitDomPipeApiEvent('domPipe:api-registry-updated', { apis: normalized, source: options.source || 'runtime' });
    return normalized;
};
domPipe.replaceApi = function (api, options = {}) {
    const normalized = normalizeDomPipeApi(api);
    if (!normalized) throw new Error('API definitions require id, method, and url.');
    this.registerApis(normalized, options);
    return normalized;
};
domPipe.getApi = function (id) { return this.apiRegistry.get(id) || null; };
domPipe.removeApi = function (id, options = {}) {
    const removed = this.apiRegistry.delete(id);
    if (removed && options.persist !== false) {
        localStorage.setItem(DOMPIPE_API_REGISTRY_STORAGE_KEY, JSON.stringify(Array.from(this.apiRegistry.values())));
    }
    if (removed) emitDomPipeApiEvent('domPipe:api-registry-updated', { removed: [id], source: options.source || 'runtime' });
    return removed;
};
domPipe.requestApi = async function (id, overrides = {}) {
    const api = this.getApi(id);
    if (!api) throw new Error(`Unknown domPipe API: ${id}`);
    const method = String(overrides.method || api.method).toUpperCase();
    if (!['GET', 'HEAD', 'POST', 'PUT', 'PATCH', 'DELETE', 'OPTIONS'].includes(method)) throw new Error(`Unsupported API method: ${method}`);
    let headers;
    try {
        headers = typeof (overrides.headers || api.headers) === 'string'
            ? JSON.parse(overrides.headers || api.headers || '{}')
            : (overrides.headers || api.headers || {});
    } catch (error) {
        throw new Error('API headers must be valid JSON.');
    }
    if (!headers || typeof headers !== 'object' || Array.isArray(headers)) throw new Error('API headers must be an object.');
    headers = Object.fromEntries(Object.entries(headers).filter(([key, value]) => /^[!#$%&'*+.^_`|~0-9A-Za-z-]+$/.test(key) && (typeof value === 'string' || typeof value === 'number' || typeof value === 'boolean')).map(([key, value]) => [key, String(value)]));
    const baseUrl = overrides.serverUrl || api.serverUrl;
    const requestUrl = baseUrl ? new URL(overrides.url || api.url, `${String(baseUrl).replace(/\/$/, '')}/`).toString() : (overrides.url || api.url);
    if (!isDomPipeSafeUrl(requestUrl)) throw new Error('API URL must use HTTP(S) or a safe relative URL.');
    if (overrides.token && !headers.Authorization && !headers.authorization) headers.Authorization = `Bearer ${overrides.token}`;
    if (!headers['Idempotency-Key'] && !headers['idempotency-key']) headers['Idempotency-Key'] = overrides.idempotencyKey || api.idempotencyKey || `domPipe-${api.id}`;
    const request = { method, headers };
    if (!['GET', 'HEAD'].includes(method)) {
        request.body = (overrides.body ?? api.body) || undefined;
    }
    const response = await fetch(requestUrl, request);
    const text = await response.text();
    let data = text;
    try { data = JSON.parse(text); } catch (error) { /* Preserve text responses. */ }
    return { response, data, text };
};

window.addEventListener('storage', event => {
    if (event.key !== DOMPIPE_API_REGISTRY_STORAGE_KEY) return;
    domPipe.apiRegistry = new Map(readDomPipeApiRegistry().map(api => [api.id, api]));
    emitDomPipeApiEvent('domPipe:api-registry-updated', { apis: Array.from(domPipe.apiRegistry.values()), source: 'storage' });
});

// XI (background interface): the dashboard, a webview, or a hollow/native
// shell can use the same live registry without owning the rendering surface.
domPipe.xi = {
    getSetup() { return Array.from(domPipe.apiRegistry.values()); },
    getAttributes() { return DOMPIPE_ATTRIBUTE_NAMES.slice(); },
    getDataAttributes() { return DOMPIPE_ATTRIBUTE_NAMES.map(domPipeDataName); },
    attributes: Object.freeze({
        dataName: domPipeDataName,
        get: getDomPipeAttribute,
        set: setDomPipeAttribute,
        remove: removeDomPipeAttribute,
        list: () => DOMPIPE_ATTRIBUTE_NAMES.slice()
    }),
    inspectDocument(root = document) {
        const elements = Array.from(root.querySelectorAll('*')).filter(element => isDomPipeElement(element));
        return elements.map(element => ({ id: element.id || null, tag: element.tagName.toLowerCase(), classes: Array.from(element.classList), attributes: Object.fromEntries(DOMPIPE_ATTRIBUTE_NAMES.filter(name => element.hasAttribute(name) || element.hasAttribute(domPipeDataName(name))).map(name => [name, getDomPipeAttribute(element, name)])), dataAttributes: Object.fromEntries(DOMPIPE_ATTRIBUTE_NAMES.filter(name => element.hasAttribute(name) || element.hasAttribute(domPipeDataName(name))).map(name => [domPipeDataName(name), getDomPipeAttribute(element, name)])) }));
    },
    subscribe(listener) {
        if (typeof listener !== 'function') return () => {};
        const handler = event => listener(event.detail || {});
        window.addEventListener('domPipe:api-registry-updated', handler);
        return () => window.removeEventListener('domPipe:api-registry-updated', handler);
    },
    replace(apis, options = {}) { return domPipe.registerApis(apis, { ...options, source: options.source || 'xi' }); },
    remove(id, options = {}) { return domPipe.removeApi(id, { ...options, source: options.source || 'xi' }); },
    connect(port) {
        if (!port || typeof port.postMessage !== 'function') throw new Error('XI connect requires a MessagePort-like object.');
        const send = detail => port.postMessage({ type: 'domPipe:api-registry-updated', detail });
        const unsubscribe = this.subscribe(send);
        send({ apis: this.getSetup(), source: 'xi-connect' });
        const receive = event => {
            const message = event?.data || event;
            if (message?.type === 'domPipe:api-registry-replace' && Array.isArray(message.apis)) this.replace(message.apis, { source: 'xi-bridge' });
            if (message?.type === 'domPipe:api-registry-remove' && message.id) this.remove(message.id, { source: 'xi-bridge' });
        };
        port.addEventListener?.('message', receive); port.start?.();
        return () => { unsubscribe(); port.removeEventListener?.('message', receive); };
    }
};
window.addEventListener('message', event => {
    const allowedOrigin = document.documentElement?.getAttribute('data-dompipe-message-origin') || window.location.origin;
    if (event.origin !== allowedOrigin) return;
    const message = event.data;
    if (message?.type === 'domPipe:api-registry-replace' && Array.isArray(message.apis)) domPipe.xi.replace(message.apis, { source: 'xi-window' });
    if (message?.type === 'domPipe:api-registry-remove' && message.id) domPipe.xi.remove(message.id, { source: 'xi-window' });
});
...
let domContentLoad = (again = false) => {
    ...
    let elementsArray_time = [
        ...document.getElementsByTagName("timer"),
        ...document.getElementsByTagName("timed")
    ];
    ...
    Array.from(elements_Carousel).forEach(function (elem) {
        ...
        setTimeout(() => carousel(elem, auto), parseInt(elem.getAttribute("delay"), 10) || 0);
    });
    ...
    Array.from(elements_mouse).forEach(function (elemv) {
        ...
        if (elemv.hasAttribute("tool-tip")) {
            ...
            var rv = eve ? eve.split(";") : ['mouseover'];
...
function loadJsonContent(column, jsonUrl) {
    ...
    // Use modal function from domPipe.js to load JSON
...
function loadGenericContent(column, contentUrl) {
    fetch(contentUrl)
        ...
        .then(content => {
            ...
            // Process any domPipe elements in the loaded content
            domContentLoad();
...
window.columnsComponent = {
    ...
    updateColumnContent: function (columnsId, columnIndex, newContent) {
        ...
        if (column) {
            ...
            domContentLoad(); // Process any domPipe elements in the new content
        }
    },
...
/**
 * refresh-component.js - Custom refresh component for domPipe.js
...
function processCheckoutTags() {
    ...
    Array.from(checkoutElements).forEach(function (checkoutElement) {
        ...
        const cart = JSON.parse(localStorage.getItem('domPipeCart') || '{"items":[]}');
...
function displayOrderSummary() {
    const cartData = localStorage.getItem('domPipeCart');
    if (!cartData) return;
...
function saveOrder(order) {
    ...
    const orders = JSON.parse(localStorage.getItem('domPipeOrders') || '[]');

    // Add new order
    orders.push(order);

    // Save back to localStorage
    localStorage.setItem('domPipeOrders', JSON.stringify(orders));
}
 ...
 */
function clearCart() {
    localStorage.setItem('domPipeCart', JSON.stringify({
        items: [],
        subtotal: 0,
        tax: 0,
        shipping: 0,
 ...
 */
function initCart() {
    if (!localStorage.getItem('domPipeCart')) {
        localStorage.setItem('domPipeCart', JSON.stringify({
            items: [],
            subtotal: 0,
            tax: 0,
...
function addToCart(productId, name, price, quantity = 1, image = '') {
    ...
    localStorage.setItem('domPipeCart', JSON.stringify(cart));
...
function renderTree(value, tempTag) {
    ...
    Object.entries(value).forEach(([k, v]) => {
        ...
        if (v instanceof Object) {
            ...
            setDomPipeSafeAttribute(temp, k, v);
        }
    });
    ...
    try {
        ...
        Object.entries(value).forEach((nest) => {
            ...
            if (v instanceof Object) {
                ...
                appendDomPipeStylesheets(v, document.head);
            }
            else if (k.toLowerCase() == "js") {
                var optsArray = v.split(";");
                // console.log(v)
...
function modala(value, tempTag, root, id) {
    ...
    // Helper: valid tag names (HTML + domPipe custom tags)
    function isValidTag(tag) {
        ...
        const domPipeTags = ["pipe", "cart", "item", "dyn", "search", "csv", "tabs", "login", "checkout", "carousel", "columns", "timer", "timed", "refresh", "order-confirmation", "lnk"];
        return htmlTags.includes(tag.toLowerCase()) || domPipeTags.includes(tag.toLowerCase());
    }
    ...
    Object.entries(value).forEach(([k, v]) => {
        ...
        if (k.toLowerCase() === "css") {
            appendDomPipeStylesheets(v, tempTag);
            return;
        }
        ...
        if (k.toLowerCase() === "append") {
            ...
            entries.forEach(entryRaw => {
                ...
                if (attrName.toLowerCase() === 'class') {
                    ...
                    setDomPipeSafeAttribute(targetElem, 'class', merged);
                } else {
                    const sep = existing && !existing.endsWith(' ') ? ' ' : '';
                    setDomPipeSafeAttribute(targetElem, attrName, existing ? existing + sep + attrValue : attrValue);
                }
            });
            return;
        }
        ...
        if (k.toLowerCase() === "sources" && (temp.tagName.toLowerCase() === "card" || temp.tagName.toLowerCase() === "carousel")) {
            ...
            setDomPipeSafeAttribute(temp, 'sources', srcStr);
...
function setTimers(target) {
    const delay = parseInt(target.getAttribute("delay"), 10);
    if (["carousel", "timer", "timed"].includes(target.tagName.toLowerCase()) && target.classList.contains("turn-auto")) {
...
function shiftFilesLeft(elem, auto = false, delay = 1000) {
    ...
    var h = 0;
    var cloneSrcs = (elem.getAttribute("sources") || '').split(";").map(source => source.trim()).filter(Boolean);
    if (!cloneSrcs.length) return;
    ...
    while (h < b) {
        ...
        var newClone = null;
        var elemType = (elem.getAttribute("type") || 'img').toLowerCase();
        if (elemType == 'audio' || elemType == 'video')
            ...
            newClone = document.createElement(elem.getAttribute("type") || 'img');
        if (newClone) {
            newClone.src = clones;
    ...
    if (auto === true || auto === "on")
        setTimeout(() => { shiftFilesLeft(elem, auto, delay); }, Number(delay) || 1000);
...
function shiftFilesRight(elem, auto = false, delay = 1000) {
    ...
    elem.setAttribute("index", newIndex);
    if (auto === true || auto === "on")
        setTimeout(() => { shiftFilesRight(elem, auto, delay); }, Number(delay) || 1000);

}

function fileShift(elem) {
    var arr = (elem.getAttribute("sources") || '').split(";").map(source => source.trim()).filter(Boolean);
    ...
    var interv = parseInt(elem.getAttribute("interval"), 10) || 1;
...
function addPipe(rootElem = document) {
    ...
    rootElem.addEventListener('click', async function (event) {
        ...
        // Only process elements with id AND ('mouse' class OR domPipe attributes OR carousel classes)
        const hasDomPipeAttr = target.hasAttribute('ajax') ||
            target.hasAttribute('modal') ||
            target.hasAttribute('inline') ||
            target.hasAttribute('event') ||
            target.hasAttribute('data-ajax') ||
            target.hasAttribute('data-modal') ||
            target.hasAttribute('data-inline') ||
            target.hasAttribute('data-event');

        if ((target.id || hasCarouselClass) && (target.classList.contains('mouse') || hasDomPipeAttr || hasCarouselClass) && !hasPipeListener(target)) {
            ...
            if (target.getAttribute('inline')) {
                ...
                domPipe.matrix[key] = domPipe.matrix[key] || {
                    inlineMacro: target.getAttribute('inline'),
                    dpVars: {},
                    element: target,
                    matrix: []
                };
                await domPipe.runInline(key, target);
            }
        }
    }, true);
...
function pipes(elem, stop = false) {
    ...
    var auto = false;
    //
    syncDomPipeAttributes(elem);
    if (elem.getAttribute('inline') == null && !elem.id)
        return;
    ...
    if (elem.hasAttribute("turn")) {
        ...
        if (index == 0) {
            ...
            if (turnElement && turnElement.hasAttribute("inline")) {
                domPipe.register();
                domPipe.runInline(turnElement.id);
            }
    ...
    if (elem.hasAttribute("headers")) {
        ...
        optsArray.forEach((e, f) => {
            ...
            headers.set(e.slice(0, separator), e.slice(separator + 1));
        });
    }
...
function renderMarkdownInline(value) {
    ...
    html = html.replace(/`([^`]+)`/g, (_, code) => {
        const token = `DOMPIPECODESLOT${codeSpans.length}TOKEN`;
        codeSpans.push(`<code>${code}</code>`);
        return token;
    });
    html = html.replace(/!\[([^\]]*)\]\((https?:\/\/[^\s)]+)\)/g, '<img src="$2" alt="$1" loading="lazy">');
    html = html.replace(/\[([^\]]+)\]\(((?:https?:\/\/|mailto:|\/)[^\s)]+)\)/g, '<a href="$2" rel="noopener noreferrer">$1</a>');
    html = html.replace(/\*\*([^*]+)\*\*/g, '<strong>$1</strong>');
    html = html.replace(/__([^_]+)__/g, '<strong>$1</strong>');
    html = html.replace(/~~([^~]+)~~/g, '<del>$1</del>');
    html = html.replace(/(^|[^*])\*([^*]+)\*/g, '$1<em>$2</em>');
    html = html.replace(/(^|[^_])_([^_]+)_/g, '$1<em>$2</em>');
    codeSpans.forEach((code, index) => { html = html.replace(`DOMPIPECODESLOT${index}TOKEN`, code); });
    return html;
}
...
function renderMarkdown(value) {
    ...
    const flushParagraph = () => {
        if (paragraph.length) { output.push(`<p>${paragraph.map(renderMarkdownInline).join('<br>')}</p>`); paragraph = []; }
    };
    ...
    lines.forEach(line => {
        if (line.trim().startsWith('```')) {
            ...
            if (!inCode) {
                ...
                output.push(`<pre><code${codeLanguage ? ` class="language-${escapeHtml(codeLanguage)}"` : ''}>`);
            } else { inCode = false; codeLanguage = ''; output.push('</code></pre>'); }
            return;
        }
        ...
        if (heading) { flushParagraph(); closeList(); output.push(`<h${heading[1].length}>${renderMarkdownInline(heading[2])}</h${heading[1].length}>`); return; }
        ...
        if (unordered || ordered) {
            ...
            if (listType !== nextList) { closeList(); listType = nextList; output.push(`<${listType}>`); }
            output.push(`<li>${renderMarkdownInline((ordered || unordered)[1])}</li>`);
            return;
        }
        const quote = line.match(/^\s*>\s?(.*)$/);
        if (quote) { flushParagraph(); closeList(); output.push(`<blockquote>${renderMarkdownInline(quote[1])}</blockquote>`); return; }
...
function addMarkdownStyles() {
    if (document.getElementById('domPipe-markdown-styles')) return;
    const style = document.createElement('style');
    style.id = 'domPipe-markdown-styles';
    style.textContent = `
        .markdown { color: inherit; line-height: 1.65; }
        .markdown > :first-child { margin-top: 0; }
        .markdown > :last-child { margin-bottom: 0; }
        .markdown h1, .markdown h2, .markdown h3, .markdown h4, .markdown h5, .markdown h6 { margin: 1.3em 0 .45em; line-height: 1.2; }
        ...
        .markdown blockquote { padding: .5em 1em; border-left: 3px solid currentColor; opacity: .78; }
        .markdown code { padding: .12em .35em; border-radius: 3px; background: rgba(127, 127, 127, .16); font-family: Consolas, monospace; font-size: .92em; }
        .markdown pre { padding: .85em 1em; overflow: auto; border-radius: 5px; background: rgba(0, 0, 0, .18); }
        .markdown pre code { padding: 0; background: transparent; }
        .markdown a { color: inherit; text-underline-offset: .15em; }
        .markdown img { max-width: 100%; height: auto; border-radius: 4px; }
        .markdown hr { margin: 1.4em 0; border: 0; border-top: 1px solid currentColor; opacity: .25; }
    `;
    document.head.appendChild(style);
}

function processMarkdownContainers() {
    const containers = document.querySelectorAll('.markdown:not([data-markdown-processed])');
    if (!containers.length) return;
    addMarkdownStyles();
    Array.from(containers).forEach(container => {
        const source = container.getAttribute('data-markdown') ?? container.textContent;
        container.innerHTML = renderMarkdown(source);
        container.setAttribute('data-markdown-processed', 'true');
    });
}

function addDomPipeVizStyles() {
    if (document.getElementById('domPipe-viz-styles')) return;
    const style = document.createElement('style');
    style.id = 'domPipe-viz-styles';
    style.textContent = `
        .viz, .viz-all { text-decoration-line: underline; text-decoration-thickness: 1px; text-underline-offset: .14em; }
        .viz-all, .viz-all * { text-decoration-line: underline; text-decoration-thickness: 1px; text-underline-offset: .14em; }
        .viz-off { text-decoration: none !important; text-decoration-line: none !important; cursor: auto !important; }
        .viz-all .viz-off, .viz-all .viz-off * { text-decoration: none !important; text-decoration-line: none !important; cursor: auto !important; }
        .viz[ajax], .viz[data-ajax], .viz.lnk, lnk.viz, .viz-all[ajax], .viz-all[data-ajax], .viz-all.lnk, lnk.viz-all { cursor: pointer; }
    `;
    document.head?.appendChild(style);
}

function processVizClasses() {
    const elements = document.querySelectorAll('.viz, .viz-all, .viz-off');
    if (!elements.length) return;
    addDomPipeVizStyles();
    elements.forEach(element => {
        element.dataset.dompipeViz = element.classList.contains('viz-off') ? 'off' : (element.classList.contains('viz-all') ? 'all' : 'self');
        if (!element.hasAttribute('aria-label') && !element.classList.contains('viz-off') && (element.hasAttribute('ajax') || element.hasAttribute('data-ajax') || element.tagName.toLowerCase() === 'lnk')) element.setAttribute('aria-label', 'Interactive link');
    });
}

const domPipeListenObservers = new WeakMap();

function getDomPipeContentDetail(element, extra = {}) {
    return {
        element,
...
function notifyDomPipeListeners(element, extra = {}) {
    if (!element) return;
    const detail = getDomPipeContentDetail(element, extra);
    element.dispatchEvent(new CustomEvent('domPipe:listen', { bubbles: true, detail }));
    window.dispatchEvent(new CustomEvent('domPipe:listen', { detail }));
}

function processListenContainers() {
    const containers = document.querySelectorAll('.listen');
    Array.from(containers).forEach(container => {
        if (domPipeListenObservers.has(container)) return;
        const observer = new MutationObserver(() => notifyDomPipeListeners(container, { source: 'mutation' }));
        observer.observe(container, { subtree: true, childList: true, characterData: true });
        domPipeListenObservers.set(container, observer);
        notifyDomPipeListeners(container, { source: 'initial' });
    });
}
